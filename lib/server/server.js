...
Meteor.methods({
  'raix:push-update': function(options) {
    ...
    if (!doc) {
      ...
      if (options.id) {
        ...
        // numbers+letters are used in Random.id() with exception of 0 and 1
          console.log('doc id test 1');
          doc._id = options.id;
          console.log('doc id test 1 A _id=',doc._id);
        ...
        doc._id = Push.appCollection._collection.insert(doc);
          console.log('doc id test 1 B _id=',doc._id);
      } else {
          console.log('doc id test 2');
        // Get the id from insert
        doc._id = Push.appCollection.insert(doc);
          console.log('doc id test 2 A _id=',doc._id);
      }
    } else {
        console.log('doc id test 3');
    ...
    if (doc && Push.debug) {
      console.log('Push: updated', doc, doc._id);
    }
